...
// myOfficerStats returns the officer_daily_stats rows for the caller
// (filtered by officer_id = JWT subject) over the last 14 days. Used
// by the police PWA's /me page so officers see their own activity
// without admin perms.
func (a *API) myOfficerStats(w http.ResponseWriter, r *http.Request) {
	c := auth.ClaimsFrom(r.Context())
	days := 14
	conn, err := a.pool.Acquire(r.Context())
	...
	rows, err := conn.Query(r.Context(),
		`SELECT s.day, s.fines_issued, s.fines_cancelled,
		        s.fines_total::text, s.unique_plates, s.anomaly_score
		   FROM officer_daily_stats s
		  WHERE s.tenant_id = $1
		    AND s.officer_id = $2::uuid
		    AND s.day >= (now() AT TIME ZONE 'UTC')::date - ($3::int * INTERVAL '1 day')
		  ORDER BY s.day DESC`,
		c.TenantID, c.Subject, days)
	if err != nil { httpx.WriteErr(w, err); return }
	defer rows.Close()
	...
	for rows.Next() {
		var it item
		if err := rows.Scan(&it.Day, &it.FinesIssued, &it.FinesCancelled,
			&it.FinesTotal, &it.UniquePlates, &it.AnomalyScore); err != nil {
